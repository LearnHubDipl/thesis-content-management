...
automatisch eine \textbf{meta.json} Datei an. Diese wird im 
selben Ordner wie die PDF-Datei auf dem Server gespeichert, 
organisiert nach Themenpool-ID unter dem Pfad 
\texttt{/app/uploads/topic-notes/\{topicPoolId\}/}. 
Sie enthält alle relevanten Infos zur Mitschrift: Titel, 
Name des Uploaders, die Keycloak-ID der hochladenden Person 
(\texttt{uploaderSub}), die E-Mail der zuständigen Lehrkraft 
(\texttt{teacherEmail}), deren Keycloak-ID 
(\texttt{teacherSub}), den Zeitstempel und den aktuellen 
Freigabestatus.
...
Bewusst wurde entschieden, die Metadaten nicht in einer 
eigenen Datenbanktabelle zu speichern. Da der Dateiname 
der PDF bereits eindeutig als Schlüssel dient und die 
meta.json direkt daneben im selben Ordner liegt, ist kein 
zusätzlicher Datenbankeintrag notwendig um eine Mitschrift 
eindeutig zu identifizieren. Das Backend liest beim Abrufen 
einer Mitschrift einfach die zugehörige meta.json ein und 
gibt deren Inhalt zusammen mit der PDF-URL ans Frontend 
zurück. Der Dateiname ist damit gleichzeitig der 
Primärschlüssel des gesamten Systems, er verbindet PDF, 
Metadaten und öffentliche URL miteinander.
...
Binärdateien \cite{docker_overview}. Die direkte Dateisystem-Speicherung ist hier performanter 
und einfacher zu verwalten, wie die Ordnerstruktur in 
Abbildung~\ref{fig:upload-ordnerstruktur} zeigt.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{pics/upload_ordnerstruktur.png}
    \caption{Ordnerstruktur der hochgeladenen Mitschriften auf dem Server}
    \label{fig:upload-ordnerstruktur}
\end{figure}
...
im Schulnetzwerk beschreibt. Der Wert 
wird über \texttt{jwt.getClaim()} direkt aus dem Token 
ausgelesen:
...
\texttt{@students.htl-leonding.ac.at} endet. 
...
\subsection{Verwaltung der Lehrkräfte}

Für das Dropdown zur Auswahl der Lehrkraft beim Upload wird 
eine vollständige Liste aller Lehrkräfte der HTL benötigt. 
In der aktuellen Implementierung ist diese Liste direkt im 
Backend als statische Liste hinterlegt und wird beim Start 
der App einmalig in den Arbeitsspeicher geladen und als 
\texttt{ConcurrentHashMap} gehalten.

Der Grund für diesen Ansatz: Lehrkräfte registrieren sich 
nicht aktiv bei LearnHub. Eine Schüler:in muss aber schon 
beim Upload eine Lehrkraft auswählen können, bevor sich 
diese jemals eingeloggt hat. Würde die Liste nur aus der 
Datenbank geladen, wäre sie am Anfang leer. Da die 
Lehrerliste der HTL Leonding weitgehend stabil ist und 
sich im Schuljahr kaum ändert, war diese Lösung für den 
Rahmen der Diplomarbeit ausreichend. 
Sie ermöglichte eine schnelle Umsetzung ohne zusätzliche 
Infrastruktur für ein eigenes Verwaltungsinterface. 
Wenn sich eine Lehrkraft zum ersten Mal einloggt, wird 
ihr Keycloak-Sub automatisch mit dem vorhandenen Datensatz 
verknüpft.

Dieser Ansatz hat jedoch einen wesentlichen Nachteil: 
Ändert sich die Liste der Lehrkräfte, zum Beispiel durch 
neue Kolleg:innen oder Abgänge, muss der Quellcode manuell 
angepasst und die Applikation neu deployed werden. Dies 
ist keine nachhaltige Lösung für den Produktivbetrieb. 
Als sinnvolle Weiterentwicklung wäre ein vollständiges 
CRUD-Interface mit Datenbankpersistenz denkbar, bei dem 
Lehrkräfte mit entsprechenden Berechtigungen die Liste 
selbst verwalten können, ohne dass ein Eingriff in den 
