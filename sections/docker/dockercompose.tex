...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{pics/compose_example.png}
    \caption{Beispiel einer einfachen Docker Compose Datei}
    \label{fig:compose-example}
\end{figure}

Das Beispiel definiert zwei Services: einen Webserver auf Basis 
von \texttt{nginx:alpine} der auf Port 80 erreichbar ist, und 
eine MySQL-Datenbank mit einem Volume zur persistenten 
Datenspeicherung. Beide Services laufen im selben Netzwerk 
und können sich gegenseitig über ihren Servicenamen erreichen.
...
\cite{docker_compose}. Gerade in der Teamarbeit ist das ein großer 
Vorteil, da alle Entwickler:innen dieselbe Umgebung verwenden und 
Konfigurationsunterschiede zwischen einzelnen Rechnern ausgeschlossen 
werden \cite{docker_compose}. Der gesamte Stack wird mit folgendem 
Befehl gestartet:
